...
@app.route('/')
def home():
    return "Bot stat service is running!"
import json
import os
import random
import asyncio
...
def save_json_compressed(data: dict, path: str):
...
def load_json_compressed(path: str):
    gz_path = path + ".gz"
...
STATS_CHANNEL_ID = -4608086383
user_stats_messages = load_json_compressed("stats.json")


from telegram import Update
from telegram.ext import ContextTypes

async def enregistrer_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    stats = {
        "points": 150,
        "level": 3
    }
    text = f"Stats de l'utilisateur {user_id} :\n" + str(stats)

    if str(user_id) in user_stats_messages:
        msg_id = user_stats_messages[str(user_id)]
        ...
        user_stats_messages[str(user_id)] = msg.message_id
        save_json_compressed(user_stats_messages, "stats.json")

    # Toujours sauvegarder même si on édite
    save_json_compressed(user_stats_messages, "stats.json")
    await update.message.reply_text("Tes stats ont été mises à jour !")

from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import ApplicationBuilder, Application, CommandHandler, ContextTypes, CallbackQueryHandler, CallbackContext, MessageHandler, filters, JobQueue
from datetime import datetime
# Nom du fichier pour sauvegarder les données
DATA_FILE = "dataa.json"
...
def save_data(data):
    with open(DATA_FILE, "w") as file:
        json.dump({str(k): v for k, v in data.items()}, file, indent=4)
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    if user_id not in players:
        print(f"Nouvel utilisateur {user_id} enregistré.")
        # Ajouter ses infos seulement s'il est vraiment nouveau
        players[user_id] = {
...
async def choose_village(update: Update, context) -> None:
    ...
    if village not in villages:
        await update.message.reply_text(f"Village invalide. Choisis un village parmi : {', '.join(villages)} essaye de commencer par la lettre majuscule 🚫")
        return
...
async def choose_clan(update: Update, context) -> None:
    ...
    if cls not in clans:
        await update.message.reply_text(f"Clan invalide. Choisis un clan parmi : {', '.join(clans)} essaye de commencer par la lettre majuscule 🚫")
        return
...
async def pvp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if enemy_id not in players:
        await update.message.reply_text("Le joueur spécifié n'existe pas parce que il n'a pas démarré le bot.")
        return
...
async def equipe(update: Update, context) -> None:
    ...
    if len(context.args) == 0:
        ...
        await update.message.reply_text("❌ Tu ne fais partie d'aucune équipe. Utilise /equipe create <nom de l'équipe> pour en créer. ⚔️")
        return
    ...
    if command == "create":
        # Créer une équipe
        if players[user_id]['level'] < 30:
            await update.message.reply_text("⚠️ Tu dois être au niveau 30 pour créer une équipe.")
            return
...
async def entrainement(update: Update, context) -> None:
    ...
    players[user_id]['money'] -= 50
    players[user_id]['exp'] += 100
    await update.message.reply_text("🏋️ Tu t'es entraîné avec 10¥ et tu as gagné 100 EXP ! Continue à progresser.")
...
async def mission_ninja_legendaire(update: Update, context) -> None:
    ...
    if players[user_id]['level'] < 60:
        await update.message.reply_text("Tu dois être au moins niveau 60 pour effectuer une mission ninja légendaire. 💪")
        return
...
async def start_auto_save(application):
    """Démarre la sauvegarde automatique toutes les heures."""
    job_queue = application.job_queue
    job_queue.run_repeating(periodic_save, interval=60, first=10)  # 3600s = 1 heure

async def auto_load(update: Update, context: CallbackContext):
    """Commande pour charger les stats des joueurs depuis le fichier JSON."""
    global players
    players = load_data()
    await update.message.reply_text("🔄 Les statistiques des joueurs ont été chargées avec succès.")

# Créer l'application
application = Application.builder().token("7593371180:AAFcKiQCbwIArKzI-skMbRvpBb8hQNa95Ls").build()
...
loop.create_task(start_auto_save(application))  # Lancement de la tâche automatique
application.add_handler(CommandHandler("auto_load", auto_load))
